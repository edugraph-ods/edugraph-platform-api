...
@router.get("/{university_id}/careers", response_model=list[CareersResponse], status_code=status.HTTP_200_OK, description="Get all careers by university id")
async def get_careers_by_university_id(
        university_id: str = Path(..., description="University ID"),
        career_repository: CareerRepository = Depends(get_career_repository)
):
    use_case = GetAllCareersByUniversityIdUseCase(career_repository)
    return await use_case.execute(university_id)